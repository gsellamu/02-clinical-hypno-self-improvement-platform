...
if (-not (Test-Path ".git")) {
    Write-Yellow "No local repository found. Initializing..."
    git init
    # Create an initial commit so branches exist
    New-Item "README.md" -ItemType File -Value "# Jeeth.ai Project" -Force > $null
    git add README.md
    git commit -m "Initial commit"
    Write-Green "Local repository initialized."
}
...
if (-not $checkRemote) {
    ...
    if (-not [string]::IsNullOrWhiteSpace($github_url)) {
        ...
        Write-Red "Warning: Proceeding without a remote. Pushing will fail later."
    }
}
...
# Temporarily allow "errors" (warnings) so the script doesn't stop
$oldPreference = $ErrorActionPreference
$ErrorActionPreference = "Continue"

git add . 2>$null  # Redirect warnings to null so they don't trigger Stop
...
if ($LASTEXITCODE -ne 0) { Write-Red "Push failed! Check your URL."; exit 1 }
